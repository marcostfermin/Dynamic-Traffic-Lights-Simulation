...
def createPIR():
    ...
    if os.path.exists("Individual Simulation Data/PIR_data.json"):
        with open("Individual Simulation Data/PIR_data.json",
                  "w") as f:  # read
...
def createCamera():
    ...
    if os.path.exists("Individual Simulation Data/Camera_data.json"):
        with open("Individual Simulation Data/Camera_data.json",
                  "w") as f:  # read
            Camera["Camera"]["carsServiced"] = data["Camera"]["carsServiced"]
            Camera["Camera"]["simulationTime"] = data["Camera"][
                "simulationTime"]
...
def createAntenna():
    ...
    if os.path.exists("Individual Simulation Data/Antenna_data.json"):
        with open("Individual Simulation Data/Antenna_data.json",
                  "w") as f:  # read
            Antenna["Antenna"]["carsServiced"] = data["Antenna"][
                "carsServiced"]
            Antenna["Antenna"]["EastToWest"] = data["Antenna"]["EastToWest"]
            Antenna["Antenna"]["NorthToSouth"] = data["Antenna"][
                "NorthToSouth"]
...
def splitAll(
):  # splits all sims into a folder having each individual sim's data separate
    createPIR()
