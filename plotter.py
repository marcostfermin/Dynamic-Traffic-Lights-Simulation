...
def plot_AWT():
    ...
    font = {
        "family":
        "serif",  # you can change the font here or remove fontdict = font in lin 31,32,33 and it'll type in the default font!
        "color": "black",
        "weight": "normal",
        "size": 16,
    }
...
def plot_Antenna():
    ...
    plt.title("Cars Served by Antenna in East-West and North-South Direction",
              fontdict=font)
    plt.grid(True)
    plt.show()
...
def show_graph():
    ...
    plt.title("Vehicles Serviced Based on Traffic Management Technology",
              fontdict=font)
    plt.grid(True)
    plt.show()
